...
========================
*/
// TODO: Clean this up, and add auto run support.
...
void G_BuildTiccmd(ticcmd_t *cmd, idUserCmdMgr *userCmdMgr, int newTics) {
    ...
    if (/*:g->joyxmove != 0  ||*/ ::g->gamekeydown[::g->key_right] || ::g->gamekeydown[::g->key_left] ||
                                  ::g->mousex != 0)
        ::g->turnheld += ::g->ticdup;
    else
        ::g->turnheld = 0;
...
void G_DoLoadLevel() {
    ...
    ::g->displayplayer = ::g->consoleplayer;
    ::g->starttime = I_GetTime();
    ::g->gameaction = ga_nothing;

    // Clear command building stuff.
...
#endif
        if (HU_Responder(ev))
            // Chat consumed the event.
            return true;
        if (ST_Responder(ev)) {
            // Status window consumed the event.
            return true;
        }
        if (AM_Responder(ev)) {
            // Automap consumed the event
            return true;
        }
    }

    if (::g->gamestate == GS_FINALE && F_Responder(ev)) {
