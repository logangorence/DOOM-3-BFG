...
idCVar hud_titlesafe("hud_titlesafe", "0.0", CVAR_GUI | CVAR_FLOAT,
                     "fraction of the screen to leave around hud for titlesafe area");
...
void idWindow::Draw(int time, float x, float y) {
    ...
    if (gui_edit.GetBool()) {
        ...
        dc->DrawText(va("x: %i  y: %i", (int) rect.x(), (int) rect.y()), 0.25, 0, dc->colorWhite,
                     idRectangle(rect.x(), rect.y() - 15, 100, 20), false);
        dc->DrawText(va("w: %i  h: %i", (int) rect.w(), (int) rect.h()), 0.25, 0, dc->colorWhite,
                     idRectangle(rect.x() + rect.w(), rect.w() + rect.h() + 5, 100, 20), false);
...
void idWindow::Redraw(float x, float y, bool hud) {
    ...
    if (flags & WIN_SHOWTIME) {
        dc->DrawText(va(" %0.1f seconds\n%s", (float) (time - timeLine) / 1000, gui->State().GetString("name")), 0.35f,
                     0, dc->colorWhite, idRectangle(100, 0, 80, 80), false);
    }

    if (flags & WIN_SHOWCOORDS) {
        dc->EnableClipping(false);
        sprintf(str, "x: %i y: %i  cursorx: %i cursory: %i", (int) rect.x(), (int) rect.y(), (int) gui->CursorX(),
                (int) gui->CursorY());
    ...
    if (gui_edit.GetBool() ||
        (flags & WIN_DESKTOP && !(flags & WIN_NOCURSOR) && !hideCursor && (gui->Active() || (flags & WIN_MENUGUI)))) {
...
intptr_t idWindow::ParseTerm(idTokenParser *src, idWinVar *var, intptr_t component) {
    ...
    const idDeclTable *table = static_cast<const idDeclTable *>( declManager->FindType(DECL_TABLE, token.c_str(),
                                                                                       false));
    if (table) {
        a = table->Index();
    ...
    for (i = 0; i < oc; i++) {
        ...
        switch (op->opType) {
            ...
            case WOP_TYPE_MOD:
                ...
                break;
            case WOP_TYPE_TABLE: {
                const idDeclTable *table = static_cast<const idDeclTable *>( declManager->DeclByIndex(DECL_TABLE,
                                                                                                      op->a));
