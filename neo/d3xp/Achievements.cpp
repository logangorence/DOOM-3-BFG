...
const struct achievementInfo_t {
    int required;
    // True if the current count is stored on the player profile.
    bool lifetime;
};

achievementInfo_t achievementInfo[ACHIEVEMENTS_NUM] =
{
...
void idAchievementManager::EventCompletesAchievement(const achievement_t eventId) {
    ...
    if (localUser == NULL || localUser->GetProfile() == NULL) {
        ...
        if (owner != NULL) {
            ...
            session->GetActingGameStateLobbyBase().SendReliableToLobbyUser(gameLocal.lobbyUserIDs[owner->entityNumber],
...
void idAchievementManager::CheckDoomClassicsAchievements(int killcount, int itemcount, int secretcount, int skill,
                                                         int mission, int map, int episode, int totalkills,
                                                         int totalitems, int totalsecret) {
    ...
    if (localUser != NULL && localUser->GetProfile() != NULL) {
        ...
        if (expansion == doom) {
            ...
            if (currentGame == DOOM_CLASSIC) {
                ...
                if (episode1completed_hard && episode2completed_hard && episode3completed_hard &&
                    episode4completed_hard) {
...
CONSOLE_COMMAND (AchievementsList, "Lists achievements and status", NULL) {
    ...
    idLocalUser *user = (player == NULL) ? session->GetSignInManager().GetMasterLocalUser()
                                         : session->GetGameLobbyBase().GetLocalUserFromLobbyUser(
                    gameLocal.lobbyUserIDs[player->GetEntityNumber()]);
    if (user == NULL) {
        idLib::Printf("Must be signed in\n");
        return;
    }
    ...
    for (int i = 0; i < ACHIEVEMENTS_NUM; i++) {
        ...
        idLib::Printf("%02d: %2d/%2d | %12.12s | %12.12s | %s%s\n", i, count, achievementInfo[i].required, pInfo, sInfo,
                      descriptionValid ? data.hidden ? "(hidden) " : "" : "(unknown) ",
                      descriptionValid ? data.name : "");
    }
}
