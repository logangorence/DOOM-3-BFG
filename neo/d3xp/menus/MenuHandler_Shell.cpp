...
//#endif
    ...
    if (nextState != state) {
        ...
        if (nextState == SHELL_STATE_PRESS_START) {
            ...
            HidePacifier();
            if (nextScreen == SHELL_AREA_START || nextScreen == SHELL_AREA_PARTY_LOBBY ||
                nextScreen == SHELL_AREA_GAME_LOBBY || nextScreen == SHELL_AREA_INVALID) {
    ...
    if (activeScreen != nextScreen) {
        ...
        if (nextScreen == SHELL_AREA_INVALID) {
            ...
            if (activeScreen > SHELL_AREA_INVALID && activeScreen < SHELL_NUM_AREAS &&
                menuScreens[activeScreen] != NULL) {
        ...
        if (session->GetActingGameStateLobbyBase().IsHost()) {
            ...
            if (!MatchTypeIsPrivate(matchParameters.matchFlags)) {
                if (Sys_Milliseconds() >= nextPeerUpdateMs) {
                    ...
                    session->GetActingGameStateLobbyBase().SendReliable(GAME_RELIABLE_MESSAGE_LOBBY_COUNTDOWN, msg,
                                                                        false);
                }
            }
        }
                ...
                } else    // binding found prompt to change
                {
                    ...
                    if (idStr::Icmp(waitBind, curBind) == 0) {
                        ...
                        if (bindScreen != NULL) {
                            ...
                            public:
                                idSWFScriptFunction_RebindKey(idMenuScreen_Shell_Bindings *_menu,
                                                              gameDialogMessages_t _msg, bool _accept,
                                                              idMenuHandler_Shell *_mgr, int _key, const char *_bind) {
                                    menu = _menu;
                                    msg = _msg;
                                    accept = _accept;
                                    mgr = _mgr;
                            ...
                            common->Dialog().AddDialog(GDM_BINDING_ALREDY_SET, DIALOG_ACCEPT_CANCEL,
                                                       new idSWFScriptFunction_RebindKey(bindScreen,
                                                                                         GDM_BINDING_ALREDY_SET, true,
                                                                                         this, sev->evValue, waitBind),
                                                       new idSWFScriptFunction_RebindKey(bindScreen,
                                                                                         GDM_BINDING_ALREDY_SET, false,
                                                                                         this, sev->evValue, waitBind),
                                                       false);
                        }
...
void idMenuHandler_Shell::SetupPCOptions() {
    ...
    if (GetPlatform() == 2 && menuBar != NULL) {
        if (g_demoMode.GetBool()) {
            ...
            if (buttonWidget != NULL) {
                buttonWidget->ClearEventActions();
                buttonWidget->AddEventAction(WIDGET_EVENT_PRESS).Set(WIDGET_ACTION_COMMAND, SHELL_CMD_MULTIPLAYER,
                                                                     index);
...
========================
*/
bool idMenuHandler_Shell::HandleAction(idWidgetAction &action, const idWidgetEvent &event, idMenuWidget *widget,
                                       bool forceHandled) {
    ...
    switch (actionType) {
        case WIDGET_ACTION_COMMAND: {
            ...
            if ((activeScreen == SHELL_AREA_GAME_LOBBY || activeScreen == SHELL_AREA_MATCH_SETTINGS) &&
                cmd != SHELL_CMD_QUIT && cmd != SHELL_CMD_MULTIPLAYER) {
                session->Cancel();
                session->Cancel();
            } else if ((activeScreen == SHELL_AREA_PARTY_LOBBY || activeScreen == SHELL_AREA_LEADERBOARDS ||
                        activeScreen == SHELL_AREA_BROWSER || activeScreen == SHELL_AREA_MODE_SELECT) &&
                       cmd != SHELL_CMD_QUIT && cmd != SHELL_CMD_MULTIPLAYER) {
                session->Cancel();
            }

            if (cmd != SHELL_CMD_QUIT &&
                (nextScreen == SHELL_AREA_STEREOSCOPICS || nextScreen == SHELL_AREA_SYSTEM_OPTIONS ||
                 nextScreen == SHELL_AREA_GAME_OPTIONS || nextScreen == SHELL_AREA_GAMEPAD ||
            ...
            switch (cmd) {
                ...
                case SHELL_CMD_CAMPAIGN: {
                    ...
                    transition = MENU_TRANSITION_SIMPLE;
                    gameLocal.GetLocalPlayer()->GetAchievementManager().LocalUser_CompleteAchievement(ACHIEVEMENT_ARTIFACT_WITH_BERSERK_PUNCH_20);
                    break;
                }
...
void idMenuHandler_Shell::UpdateBGState() {
    ...
    if (smallFrameShowing) {
        if (nextScreen != SHELL_AREA_PLAYSTATION && nextScreen != SHELL_AREA_SETTINGS &&
            nextScreen != SHELL_AREA_CAMPAIGN && nextScreen != SHELL_AREA_DEV) {
            if (nextScreen != SHELL_AREA_RESOLUTION && nextScreen != SHELL_AREA_GAMEPAD &&
                nextScreen != SHELL_AREA_DIFFICULTY && nextScreen != SHELL_AREA_SYSTEM_OPTIONS &&
                nextScreen != SHELL_AREA_GAME_OPTIONS && nextScreen != SHELL_AREA_NEW_GAME &&
                nextScreen != SHELL_AREA_STEREOSCOPICS && nextScreen != SHELL_AREA_CONTROLS) {
                ShowSmallFrame(false);
            }
        }
    } else {
        if (nextScreen == SHELL_AREA_RESOLUTION || nextScreen == SHELL_AREA_GAMEPAD ||
            nextScreen == SHELL_AREA_PLAYSTATION || nextScreen == SHELL_AREA_SETTINGS ||
            nextScreen == SHELL_AREA_CAMPAIGN || nextScreen == SHELL_AREA_CONTROLS || nextScreen == SHELL_AREA_DEV ||
    ...
    if (largeFrameShowing) {
        if (nextScreen != SHELL_AREA_PARTY_LOBBY && nextScreen != SHELL_AREA_GAME_LOBBY &&
            nextScreen != SHELL_AREA_CONTROLLER_LAYOUT && nextScreen != SHELL_AREA_KEYBOARD &&
            nextScreen != SHELL_AREA_LEADERBOARDS && nextScreen != SHELL_AREA_MATCH_SETTINGS &&
            nextScreen != SHELL_AREA_MODE_SELECT && nextScreen != SHELL_AREA_BROWSER && nextScreen != SHELL_AREA_LOAD &&
            nextScreen != SHELL_AREA_SAVE && nextScreen != SHELL_AREA_CREDITS) {
            ShowMPFrame(false);
        }
    } else {
        if (nextScreen == SHELL_AREA_PARTY_LOBBY || nextScreen == SHELL_AREA_CONTROLLER_LAYOUT ||
            nextScreen == SHELL_AREA_GAME_LOBBY || nextScreen == SHELL_AREA_KEYBOARD ||
            nextScreen == SHELL_AREA_LEADERBOARDS || nextScreen == SHELL_AREA_MATCH_SETTINGS ||
            nextScreen == SHELL_AREA_MODE_SELECT || nextScreen == SHELL_AREA_BROWSER || nextScreen == SHELL_AREA_LOAD ||
            nextScreen == SHELL_AREA_SAVE || nextScreen == SHELL_AREA_CREDITS) {
            ShowMPFrame(true);
...
void idMenuHandler_Shell::ShowDoomIntro() {
    ...
    if (introGui != NULL) {
        ...
        idStr textEntries[NUM_DOOM_INTRO_LINES] = {
                ...
                idLocalization::GetString("#str_03012"), idLocalization::GetString("#str_04056"),
        ...
        for (int i = 0; i < numTextFields; ++i) {
            ...
            if (infoSprite != NULL && txtVal != NULL) {
                ...
                public:
                    idIntroTextUpdate(idSWFTextInstance *_txtVal, int _numLines, int _nextIndex,
                                      idMenuHandler_Shell *_shell, idSWF *_gui) {
                        txtVal = _txtVal;
                        generating = false;
                        numLines = _numLines;
                        ...
                        if (!generating) {
                            ...
                            if (!txtVal->generatingText) {
                                ...
                                if (newYPos <= 350.0f - (numLines * 36.0f)) {
                                    ...
                                    if (nextIndex < NUM_DOOM_INTRO_LINES) {
                                        idSWFSpriteInstance *nextInfo = gui->GetRootObject().GetNestedSprite(
                                                va("info%d", nextIndex));
                                        if (nextInfo != NULL &&
                                            nextInfo->GetCurrentFrame() != nextInfo->FindFrame("active")) {
                ...
                infoSprite->GetScriptObject()->Set("onEnterFrame",
                                                   new idIntroTextUpdate(txtVal, txtVal->CalcNumLines(), i + 1, this,
                                                                         introGui));
            }
        }
...
void idMenuHandler_Shell::ShowROEIntro() {
    ...
    if (introGui != NULL) {
        ...
        idStr textEntries[NUM_ROE_INTRO_LINES] = {idLocalization::GetString("#str_00100870"),
                                ...
                                if (nextIndex % 2 != 0) {
                                    if (nextIndex < NUM_ROE_INTRO_LINES) {
                                        ...
                                        if (nextInfo != NULL &&
                                            ...
                                            idSWFTextInstance *txtData = nextInfo->GetScriptObject()->GetNestedText(
                                                    "txtInfo", "txtVal");
                                                ...
                                                if (startFade == 0) {
                                                    ...
                                                    if (Sys_Milliseconds() - startFade >= 3000) {
                                                        ...
                                                        if (nextDateIndex < NUM_ROE_INTRO_LINES) {
                                                            idSWFSpriteInstance *nextInfo = gui->GetRootObject().GetNestedSprite(
                                                                    va("info%d", nextDateIndex));
                                                            if (nextInfo != NULL && nextInfo->GetCurrentFrame() !=
                                                                                    nextInfo->FindFrame("active")) {
                                                                nextInfo->StopFrame("active");
                                                                return idSWFScriptVar();
                                                            }
                                                        } else {
                                                        ...
                                                        float alpha = 1.0f - ((float) (Sys_Milliseconds() - startFade) /
                                                                              3000.0f);
...
void idMenuHandler_Shell::ShowLEIntro() {
    ...
    if (introGui != NULL) {
        ...
        idStr textEntry = va("%s\n%s\n%s", idLocalization::GetString("#str_00200071"),
                             idLocalization::GetString("#str_00200072"), idLocalization::GetString("#str_00200073"));
