...
// Think flags
enum {
    TH_ALL = -1, TH_THINK = 1,        // run think function each frame
...
public:
    ...
    struct entityFlags_s {
        ...
        bool solidForTeam
                : 1;    // if true this entity is considered solid when a physics team mate pushes entities
        ...
        bool hasAwakened
                : 1;    // before a monster has been awakened the first time, use full PVS for dormant instead of area-connected
...
ABSTRACT_PROTOTYPE(idEntity);
    ...
    bool StartSound(const char *soundName, const s_channelType channel, int soundShaderFlags, bool broadcast,
                    int *length);
    ...
    enum {
        EVENT_STARTSOUNDSHADER, EVENT_STOPSOUNDSHADER, EVENT_MAXEVENTS
    };
    ...
    void ServerSendEvent(int eventId, const idBitMsg *msg, bool saveEvent,
                         lobbyUserID_t excluding = lobbyUserID_t()) const;
    ...
    enum interpolationBehavior_t {
        USE_NO_INTERPOLATION, USE_LATEST_SNAP_ONLY, USE_INTERPOLATION
    };
...
CLASS_PROTOTYPE(idAnimatedEntity);
    ...
    bool GetJointTransformForAnim(jointHandle_t jointHandle, int animNum, int currentTime, idVec3 &offset,
                                  idMat3 &axis) const;
    ...
    void AddLocalDamageEffect(jointHandle_t jointNum, const idVec3 &localPoint, const idVec3 &localNormal,
                              const idVec3 &localDir, const idDeclEntityDef *def, const idMaterial *collisionMaterial);
    ...
    enum {
        EVENT_ADD_DAMAGE_EFFECT = idEntity::EVENT_MAXEVENTS, EVENT_MAXEVENTS
    };
